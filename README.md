...
当前补丁程序版本：`0.5.11`，桌面运行载荷版本：`1.2.11`。

`0.5.11` 补充当前 Figma 的剪贴板提示、文件夹跳转、搜索框、变量面板和资源分类文案。截图中的 12 个英文标签均由内置精确词库覆盖，可在首帧直接命中；`0.5.10` 的大型属性面板预翻译和既有性能优化继续保留。升级后需重新应用补丁并重启 Figma。
