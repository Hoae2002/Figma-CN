...
当前补丁程序版本：`0.5.6`，桌面运行载荷版本：`1.2.6`。

`0.5.6` 移除“不翻译的区域”及历史排除规则，避免旧规则继续静默阻止词库汉化；补齐尺寸约束菜单、颜色库空状态等词条，并扩展 Figma 浮层、提示和搜索占位符的识别。字体族、插件／工具名称、用户输入和设计内容继续保持原文。升级后需重新应用补丁并重启 Figma。
