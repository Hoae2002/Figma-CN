...
当前补丁程序版本：`0.5.4`，桌面运行载荷版本：`1.2.4`。

`0.5.4` 将在线翻译严格限制在 Community 页面，并增加独立开关。编辑器、文件页、页面菜单和桌面菜单只使用内置词库；社区仍先匹配词库，未命中文案才使用 Google，返回结果会经过 Figma／UI 设计术语校正。旧版在非社区产生的机器缓存不再回放。升级后需重新应用补丁并重启 Figma。
...
5. 点击“点选排除”后，可以先正常打开下拉菜单或右键菜单，再点击要排除的项目；按住 Alt 选择上层区域，按住 Shift 可直接选择弹层触发按钮，Esc 退出。规则优先使用所选控件或父级的稳定标识；没有稳定标识时按同一区域的同类文案保存。
6. 也可以直接排除整个社区页面、工具栏、页面菜单、属性面板等区域。排除列表中的“恢复翻译”可撤销该规则。Figma 改版导致标识变化时可能需要重新选择。

**保护边界：**不改写画布、输入值、组件变体值、图层/文件/项目/组件等用户命名、字体和代码。第三方插件、无法识别的新区域、可能包含用户内容的动态段落保留原文，不发送翻译。安全识别仍需随 Figma 页面结构更新，不能保证所有英文都能覆盖。原生菜单只使用内置词库；系统弹窗的标题、正文和动态详情保持原文。
...
缓存和排除设置保存在 `%LOCALAPPDATA%\FigBoost\translation\cache.json`，备份为 `cache.json.bak`，最多保留 10,000 条机器结果。旧版在非社区区域产生的机器缓存会保留在文件中以便回退，但新版不会读取或显示这些结果；新产生的机器缓存只有社区文案。旧版 `state.json` 及加密凭据不再读取。更新和卸载默认保留个人数据。内置词库随桌面载荷发布并优先匹配，不会覆盖或改写用户缓存文件。
