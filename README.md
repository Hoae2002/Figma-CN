...
当前补丁程序版本：`0.5.2`，桌面运行载荷版本：`1.2.2`。

`0.5.2` 修复当前 Figma 根节点的画布功能标记误伤整页的问题，并补齐文件首页、工具栏和左右面板的普通标签识别；文件名、页面与图层名称、画布内容仍保持原文。自动汉化无需密钥：界面文字经 Google 网页翻译后自动缓存；设置只保留汉化开关和区域排除。升级后需重新应用补丁并重启 Figma。
