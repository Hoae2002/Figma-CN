...
当前补丁程序版本：`0.5.13`，桌面运行载荷版本：`1.2.13`。

`0.5.13` 补充文件夹权限、分享范围、评论、Widget 和左侧导航等当前 Figma 界面词条，并修复这些系统命令因同时处于文件链接、评论或插件容器中而被用户内容保护规则跳过的问题。含文件夹名称的权限说明只翻译固定界面文案，动态名称保持原文。升级后需重新应用补丁并重启 Figma。
