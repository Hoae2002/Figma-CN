...
当前补丁程序版本：`0.5.10`，桌面运行载荷版本：`1.2.10`。

`0.5.10` 修复大型属性面板重建时仍可能闪现英文的问题。普通 Figma 页面会先以常数时间查询内置精确词库，并在首帧前应用命中的安全界面标签；未知字段、短语规则和社区在线补译继续进入有预算的后台处理。`0.5.9` 的 4ms 预绘制时间片和 `0.5.8` 的性能优化继续保留。升级后需重新应用补丁并重启 Figma。
