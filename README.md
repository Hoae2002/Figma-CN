...
当前补丁程序版本：`0.5.8`，桌面运行载荷版本：`1.2.8`。

`0.5.8` 完成第二轮性能修复：DOM 监听、浮层识别和译文布局标记不再读取计算样式或元素尺寸，完整候选与内容保护规则对每段文字只执行一次；批量界面更新已有性能回归测试约束。`0.5.7` 及此前的词条、内容保护和社区补译行为继续保留。升级后需重新应用补丁并重启 Figma。
