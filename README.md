...
当前补丁程序版本：`0.5.7`，桌面运行载荷版本：`1.2.7`。

`0.5.7` 修复开启汉化后编辑器明显卡顿的问题。浮层识别不再在每次文本扫描时读取计算样式，改为只检查 DOM 语义和稳定标记；新增回归测试，禁止翻译策略在高频路径触发布局或样式计算。`0.5.6` 的词条补充、旧排除规则移除和内容保护继续保留。升级后需重新应用补丁并重启 Figma。
