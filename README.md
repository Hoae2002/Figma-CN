...
当前补丁程序版本：`0.5.5`，桌面运行载荷版本：`1.2.5`。

`0.5.5` 恢复并加强设计资源保护：右侧 Tools 区域中的插件／工具名称、字体族当前值及字体下拉选项不再经过词库或 Google。页面、样式、导出、工具、排版等 Figma 系统标题仍按词库汉化。保护判断同时使用控件语义、区域结构和字体特征，减少 Figma 改版造成的选择器失效。升级后需重新应用补丁并重启 Figma。
